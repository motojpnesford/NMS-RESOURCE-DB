# NMS Resource DB
「自分が欲しかったのは、資源の増やし方がすぐ分かるアプリだった」

No Man's Sky 日本語版向けの「資源の増やし方」に特化したシンプルな検索データベースです。

---

## コンセプト

ゲーム中に

「この資源、どうやって増やすんだっけ？」

と思った時に、数秒で確認できることを目的としています。

攻略サイトを開くより速く、必要な情報だけを表示することを目指しています。

---

## 現在の機能

- 資源名検索
- Enterキーで検索
- recipes.json読込
- 増殖レシピ表示
- 日本語版正式名称対応

---

## データ登録ルール

- 日本語版ゲーム内正式名称を使用
- 最新ゲームバージョン準拠
- ゲーム内確認を最優先とする
- 精製機の種類を表示
- JSON形式を統一

---

## 登録済みグループ（予定）

### 第1グループ（基本資源）

- 炭素
- 濃縮炭素
- 酸素
- フェライト塵
- 純粋フェライト
- 磁化フェライト
- 有色金属

### 第2グループ（元素）

- 銅
- カドミウム
- エメリル
- インジウム

### 第3グループ（鉱物）

- コバルト
- イオン化コバルト
- リン
- パラフィニウム
- アンモニア
- 二酸化物
- ウラン
- 黄鉄鉱

### 第4グループ

登録中

---

## 今後の予定

- 第4グループ完成
- ゲーム内レシピ確認
- 「増殖できません」表示機能
- 部分一致検索
- 材料から検索
- おすすめレシピ表示
- No Man's Sky風UI改善

---

## 使用技術

- HTML
- CSS
- JavaScript
- JSON
- GitHub Pages

---
# NMS RESOURCE DB

No Man's Sky の資源データベース。

## 現在のバージョン

Ver.1.0 (開発中)

---

## 本日の進捗

### データ構造を全面的に見直し

従来の message 中心の構造を廃止し、役割ごとにデータを分割。

現在のJSON形式

```json
{
    "name": "",
    "guide": [],
    "recipes": [],
    "convert": [],
    "collect": [],
    "use": [],
    "note": []
}
```

---

### script.js 改修

表示処理を関数ごとに分割。

実装済み

- escapeHtml()
- createSection()
- showGuide()
- showRecipes()
- showConvert()
- showCollect()
- showUse()
- showNote()

showResult() は各表示関数を呼び出す構成へ変更。

---

### 今後の方針

JSONにはデータのみを記述し、
表示方法はすべてJavaScript側で管理する。

これにより、

- デザイン変更
- 検索機能追加
- タップ検索追加

などをJSONを書き換えることなく実装可能となった。

## Overview

# Version

## Ver.1.0 Development

### Data format update

資源データ形式を正式フォーマットへ変更。

各資源JSONは以下の項目で管理する。

- name
- category
- guide
- recipes
- convert
- collect
- use
- note

---

## Guide format

おすすめ増殖手順は以下の形式で管理。

```json
{
    "step": "①",
    "title": "手順タイトル",
    "text": "説明文"
}

```

---

##今後
Data registration policy

今後の資源データ登録は専用ページ情報を基準にする。

登録ルール：

Wiki専用ページを確認
精製レシピは可能な限り全て登録
おすすめルートのみguideへ登録
入手方法・用途・補足も記録
Current registered resources
Basic resources
炭素
濃縮炭素
Development notes

Ver.1.0ではデータ量増加を前提として設計。

将来的に、

部分一致検索
検索候補表示
資源名クリックによる関連検索
精製素材ツリー表示

などの機能追加を予定。

# NMS RESOURCE DB

## 現在の進捗

### Ver.1.0 データ登録

#### 基本資源
- 炭素
- 濃縮炭素
- 酸素
- フェライト塵
- ピュアフェライト
- 磁化フェライト

#### 恒星金属
- 銅
- 有色金属
- カドミウム
- エメリル
- インジウム

---

## JSONルール（Ver.1.0正式）

全資源共通

- name
- category
- guide
- recipes
- convert
- collect
- use
- note

guideは攻略情報。

recipesは「その資源を作る方法」のみ。

convertは「その資源から作れるもの」。

collectは主な入手方法。

useは代表的な用途のみ。

noteは攻略に役立つ補足情報。

# README.md 更新内容

## NMS RESOURCE DB Ver.0.10

### 本日の追加データ

#### 鉱物資源

* 塩
* 塩素
* 金
* 銀
* プラチナ

#### 高エネルギー物質

* 三重水素
* 重水素
* 二水素

### データベースの改善

* 鉱物資源系を正式フォーマット（Ver.1.0）で登録。
* guide / recipes / convert / collect / use / note の構成を統一。
* すべてWiki専用ページを基準として情報を整理。
* JSON登録順は動作に影響しないことを確認。

### 現在登録済み

#### 基本資源

* 炭素
* 濃縮炭素
* 酸素
* フェライト塵
* ピュアフェライト
* 磁化フェライト

#### 恒星金属

* 銅
* 有色金属
* カドミウム
* エメリル
* インジウム

#### 鉱物資源

* コバルト
* イオン化コバルト
* 塩
* 塩素
* 金
* 銀
* プラチナ

#### 高エネルギー物質

* 二水素
* 三重水素
* 重水素

#### 局在化アース資源

* リン
* パラフィン
* アンモニア
* 二酸化物
* ウラン
* 黄鉄鉱

---

## 次回予定

植物資源グループの登録開始

予定順

1. フロストクリスタル
2. ソラニウム
3. 真菌カビ
4. サボテン果肉
5. スターバルブ
6. ガンマルート

局在化アース資源との相互変換ルートを完成させる。
## 開発状況（2026-08-01）

### 新規登録
#### ジャンク資源
- 残留粘体
- どろりとした液体
- 動く粘液
- 暴走カビ

#### 通貨
- ナノマシンクラスター

#### 圧縮大気ガス
- 窒素
- ラドン
- サルファリン

### データベース進捗
現在登録済みカテゴリ

- 基本資源
- 恒星金属
- 鉱物資源
- 植物資源
- 特殊資源
- ジャンク
- 通貨
- 圧縮大気ガス

資源同士の精製ルートが充実し、検索から関連資源へ辿れるデータベースとして完成度が向上。
## ライセンス

個人利用

# NMS RESOURCE DB

## 開発状況（2026-08-03）

### 完了
- アロニウム登録
- マグノゴールド登録
- グランティン登録
- イリデサイト登録

### 達成
- イリデサイトクラフトルート完成

### 高額クラフト

融合点火装置ルート完成

ステイシスデバイスルート完成

### 次回予定

- 素材名クリックによる逆引き検索実装
- 検索結果内の素材名をクリックすると、その素材を自動検索する機能を追加
- 実装後、データ収集を再開
